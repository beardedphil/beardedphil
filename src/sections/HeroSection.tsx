...
function SocialIcon({ label }: { label: string }) {
  switch (label) {
    case "LinkedIn":
      return (
        <svg className="hero-social-icon" viewBox="0 0 24 24" aria-hidden>
          <path
            fill="currentColor"
...
function SocialLink({ cta }: { cta: CtaLink }) {
  const external = cta.external
    ? { target: "_blank" as const, rel: "noopener noreferrer" as const }
    : {};
  return (
    <a className="hero-social-link" href={cta.href} {...external}>
      <SocialIcon label={cta.label} />
      <span>{cta.label}</span>
    </a>
  );
}
    ...
    <section className="section hero fade-in-up">
      <div className="hero-layout">
        ...
        <div className="hero-content-column">
          <nav className="hero-social" aria-label="LinkedIn, resume, and GitHub">
